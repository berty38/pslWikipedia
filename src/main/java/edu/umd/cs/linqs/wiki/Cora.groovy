...
ConfigBundle coraBundle = cm.getBundle("cora")

def defaultPath = System.getProperty("java.io.tmpdir")
String dbpath = coraBundle.getString("dbpath", defaultPath + File.separator + "psl")
DataStore data = new RDBMSDataStore(new H2DatabaseDriver(Type.Disk, dbpath, true), coraBundle)
...
for (int fold = 0; fold < folds; fold++) {
	...
	dbPop = new DatabasePopulator(db);
	dbPop.populate(new QueryAtom(avgValue__1, Document, Category), substitutions)
	ConfigBundle tightEqualityConfig = cm.getBundle("cora");
	...
	for (String method : methods) {
		 ...
		 * Weight learning
		 */
		learn(m, trainDB, labelsDB, coraBundle, method, log)
		...
		for (RandomVariableAtom atom : Iterables.filter(allAtoms, RandomVariableAtom))
			atom.setValue(0.0)
		MPEInference mpe = new MPEInference(m, testDB, coraBundle)
